...
const MuzzleCompany = () => {
  ...
  const role = "Muzzle Company"
  const roleSingular = "Muzzle Company"
  ...
  useEffect(() => {
    let headers = [
      "Id,Name,Mobile,Category,Address",
    ];
    ...
    if (Array.isArray(userAnimalsCSV.data) && userAnimalsCSV.data.length > 0) {
      ...
      downloadFile({
        ...
        fileName: "Muzzle Company List.csv",
        fileType: "text/csv",
      });
    }
  }, [userAnimalsCSV]);
  ...
  const exportToCsv = (userType) => {
    let requestUrl = "user/get-company";
    apiRequest({
      url: requestUrl,
      method: "POST",
  ...
  const acceptAceRequest = () => {
    apiRequest({
      url: `user/deleteCompany/${openAceRequestModal.data.company_id}`,
      method: "DELETE",
    })
      .then((res) => {
  ...
  return (
    <Box>
      <CustomDialog
        ...
        title={currentAnitraUserState?.company_id ? "Edit Company" : "Add Company"}
      >
        <AddEditUser
          onCancel={() => setOpenUserModal({ open: false })}
          roleSingular={roleSingular}
          currentState={currentAnitraUserState}
          isEdit={Boolean(currentAnitraUserState?.company_id)}
          getUsersByType={() =>
